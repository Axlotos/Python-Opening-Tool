...
Python Opening Tool is a program that can be used to open Python Source Files. When you open a Python file with Python Opening Tool it will open a window with three buttons. One button opens your file in VS Code, one opens it in Visual Studio, one that opens it in an app you choose, and one runs the file. There is also an option to change the theme from light and dark mode. The custom app and theme save! This is what the theme.txt and customAppPath.txt files are for.
...
This also creates two new folders, one called obj and one called bin. Go into bin\Release\net10.0-windows7.0\publish, you will see the compiled code. Copy ALL the files and paste them into a safe spot like a folder on your C: drive call PythonOpeningTool. You also need to copy and paste visualStudioPath.txt, theme.txt, and customAppPath.txt (from the same directory as this README) into that folder.
